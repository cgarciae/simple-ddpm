...
class UNet2DModule(nn.Module):
    ...
    def __call__(self, sample, timesteps, *, deterministic=True):
        ...
        for down_block in self.down_blocks:
            if isinstance(down_block, CrossAttnDownBlock2D):
                sample, res_samples = down_block(
                    sample, t_emb, deterministic=deterministic
                )
            else:
                sample, res_samples = down_block(
                    sample, t_emb, deterministic=deterministic
        ...
        # 4. mid
        sample = self.mid_block(sample, t_emb, deterministic=deterministic)
